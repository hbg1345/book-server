-- Initial schema. Flyway runs this exactly once on a fresh database and records it
-- in flyway_schema_history, so there are no DROP statements here (unlike the test
-- schema.sql, which is re-applied per test and therefore drops first).
